...
export function Topbar({ onMenuClick }) {
  ...
  return (
    <div className="h-[60px] sm:h-[72px] shrink-0 border-b border-white/5 bg-[#0f1629]/80 backdrop-blur-md sticky top-0 z-40 flex items-center justify-between px-3 sm:px-6 gap-2">
      ...
      <div className="flex items-center gap-2 sm:gap-4 min-w-0">
        ...
        <button
          onClick={onMenuClick}
          className="lg:hidden flex-shrink-0 w-9 h-9 flex flex-col justify-center items-center gap-1.5 text-[#8892b0] hover:text-white transition-colors"
      ...
      <div className="flex items-center gap-2 sm:gap-4 flex-shrink-0">
        ...
        <div className="hidden sm:flex items-center gap-2">
          ...
          <span className="font-orbitron font-bold text-[#e8eaf6] tracking-wider text-xs sm:text-sm whitespace-nowrap">{dateStr}</span>
        </div>
        ...
        <motion.button
          ...
          className={`px-3 sm:px-5 py-2 sm:py-2.5 rounded-lg font-bold text-xs sm:text-sm tracking-wide transition-all shadow-lg flex items-center gap-1 sm:gap-2 whitespace-nowrap
            ${hasMatch
              ? 'bg-gradient-to-r from-[#00c8ff] to-[#0090b8] text-white shadow-[0_0_15px_rgba(0,200,255,0.4)] border border-white/20'
