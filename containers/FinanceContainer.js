  ...
  return (
    <div className="pb-10 min-h-screen">
      <div className="mb-5 sm:mb-6">
        ...
        <p className="text-[#8892b0] text-sm sm:text-[15px]">Finans, Sponsorluk ve Tesis Yönetimi</p>
      </div>
        ...
        {tabs.map(tab => (
          <button
            ...
            className={`flex items-center gap-2 px-5 py-3 rounded-xl font-bold text-sm tracking-wider uppercase transition-all whitespace-nowrap
              ${activeTab === tab.id 
                ? 'bg-[#00c8ff] text-black shadow-[0_0_15px_rgba(0,200,255,0.4)] scale-105' 
      ...
      <AnimatePresence mode="wait">
        ...
        {activeTab === 'overview' && (
          ...
          >
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
              {/* Total Balance Card */}
              <div className="bg-[#141b2d]/80 backdrop-blur-md rounded-2xl border border-[#00e676]/30 shadow-[0_10px_30px_rgba(0,230,118,0.1)] p-6 relative overflow-hidden">
              ...
              <div className="bg-[#141b2d]/80 backdrop-blur-md rounded-2xl border border-[#00c8ff]/30 shadow-[0_10px_30px_rgba(0,200,255,0.1)] p-6 relative overflow-hidden">
              ...
              {/* Wage Budget Card */}
              <div className="bg-[#141b2d]/80 backdrop-blur-md rounded-2xl border border-[#f5c842]/30 shadow-[0_10px_30px_rgba(245,200,66,0.1)] p-6 relative overflow-hidden">
                ...
                <div className="font-orbitron font-bold text-3xl sm:text-4xl text-[#f5c842] drop-shadow-[0_0_15px_rgba(245,200,66,0.4)] mb-2">
            ...
            <div className="bg-[#141b2d]/80 backdrop-blur-md rounded-2xl border border-white/5 shadow-lg overflow-hidden">
              <div className="px-6 py-4 border-b border-white/5 bg-black/20 flex justify-between items-center">
                <span className="font-rajdhani font-bold text-lg tracking-wider text-[#e8eaf6] uppercase">Sezonluk Bilanço & Akış</span>
              </div>
              ...
              <div className="p-6">
                ...
                <div className="mt-8">
                  <h4 className="text-[#8892b0] font-bold text-sm uppercase tracking-wider mb-4">Son Finansal İşlemler</h4>
                  <div className="space-y-2 max-h-60 overflow-y-auto custom-scrollbar pr-2">
                    {(finances?.history || []).slice(0, 10).map((h, i) => (
                      <div key={i} className="flex items-center justify-between p-3 rounded-lg bg-white/5 border border-white/5">
                        ...
                        <span className={`font-orbitron font-bold text-sm ${h.type === 'income' ? 'text-[#00e676]' : 'text-[#ff1744]'}`}>
        ...
        {activeTab === 'sponsors' && (
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {/* Forma Sponsoru */}
              <div className="bg-[#141b2d]/80 rounded-2xl border border-white/5 p-6">
                  ...
                  <div className="bg-gradient-to-r from-blue-900/40 to-black/40 p-5 rounded-xl border border-blue-500/30 text-center relative overflow-hidden">
                    ...
                    <div className="flex justify-center gap-6 mt-4 relative z-10">
                      <div>
                        ...
                        <div className="text-[#00e676] font-bold text-lg">+{formatMoney(finances.sponsors.shirt.valuePerWeek)}</div>
                      </div>
                      <div>
                ...
                {finances?.sponsors?.stadium ? (
                  <div className="bg-gradient-to-r from-red-900/40 to-black/40 p-5 rounded-xl border border-red-500/30 text-center relative overflow-hidden">
                    ...
                    <div className="flex justify-center gap-6 mt-4 relative z-10">
                      <div>
                        <div className="text-[10px] text-white/50 uppercase tracking-wider">Haftalık Gelir</div>
                        <div className="text-[#00e676] font-bold text-lg">+{formatMoney(finances.sponsors.stadium.valuePerWeek)}</div>
                      </div>
                      <div>
                        <div className="text-[10px] text-white/50 uppercase tracking-wider">Kalan Süre</div>
            ...
            {/* Gelen Teklifler */}
            <div className="bg-[#141b2d]/80 rounded-2xl border border-white/5 overflow-hidden">
              <div className="px-6 py-4 border-b border-white/5 bg-black/20 flex justify-between items-center">
                <span className="font-rajdhani font-bold text-lg tracking-wider text-[#e8eaf6] uppercase flex items-center gap-2">📩 Gelen Sponsor Teklifleri</span>
                <span className="bg-[#00c8ff]/20 text-[#00c8ff] px-3 py-1 rounded-lg text-xs font-bold">{finances?.sponsors?.offers?.length || 0} Teklif</span>
              </div>
              <div className="p-6">
                {(!finances?.sponsors?.offers || finances.sponsors.offers.length === 0) ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {finances.sponsors.offers.map(offer => (
                      <div key={offer.id} className="bg-black/40 border border-[#f5c842]/30 p-5 rounded-xl hover:border-[#f5c842] transition-colors relative">
                        <div className="absolute top-3 right-3 text-[10px] bg-[#f5c842]/20 text-[#f5c842] px-2 py-1 rounded uppercase font-bold">
                          {offer.type === 'shirt' ? 'Forma' : 'Stadyum'}
                        </div>
                        <h4 className="font-rajdhani font-bold text-xl text-white mb-1 truncate">{offer.name}</h4>
                        <div className="text-sm text-[#8892b0] mb-4">Yeni sponsorluk teklifi gönderdi.</div>
                        
                        <div className="grid grid-cols-2 gap-4 mb-5">
                          <div className="bg-white/5 rounded p-2 text-center">
                            <div className="text-[10px] text-white/50 uppercase">İmza Parası</div>
                            <div className="font-bold text-[#00e676]">{formatMoney(offer.bonus)}</div>
                          </div>
                          <div className="bg-white/5 rounded p-2 text-center">
                            <div className="text-[10px] text-white/50 uppercase">Haftalık Ücret</div>
                            <div className="font-bold text-[#00c8ff]">{formatMoney(offer.valuePerWeek)}</div>
                          </div>
                        ...
                        <button 
                          ...
                          className="w-full bg-gradient-to-r from-[#00c8ff] to-[#0090b8] text-white py-2 rounded-lg font-bold text-sm tracking-widest uppercase hover:scale-[1.02] transition-transform"
                        >
                          Teklifi Kabul Et
                        </button>
                      </div>
        ...
        {activeTab === 'facilities' && (
          ...
          >
            {[
              ...
              return (
                <div key={fac.id} className="bg-[#141b2d]/80 rounded-2xl border border-white/5 p-6 flex flex-col relative overflow-hidden">
                  ...
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-xs font-bold text-[#00c8ff] uppercase tracking-wider">Seviye {currentLevel} / 5</div>
                      </div>
                    </div>
                  </div>
                  <p className="text-sm text-[#8892b0] mb-6 flex-1">{fac.desc}</p>
                  
                  {currentLevel >= 5 ? (
                    <div className="bg-[#00e676]/10 text-[#00e676] border border-[#00e676]/20 p-3 rounded-xl text-center font-bold text-sm">
        ...
        {activeTab === 'events' && (
          ...
          >
            ...
            <div className={`rounded-2xl border p-6 sm:p-8 transition-colors ${finances?.cfoHired ? 'bg-[#00c8ff]/10 border-[#00c8ff]/50' : 'bg-[#141b2d]/80 border-white/5'}`}>
              <div className="flex flex-col sm:flex-row gap-6 items-center justify-between">
                <div className="flex items-center gap-4">
                  <div className={`w-16 h-16 rounded-2xl flex items-center justify-center text-3xl border ${finances?.cfoHired ? 'bg-[#00c8ff]/20 border-[#00c8ff] text-[#00c8ff]' : 'bg-white/5 border-white/10 text-white/50'}`}>
                    🤖
                  </div>
                  <div>
                    <h3 className={`font-rajdhani font-bold text-xl sm:text-2xl mb-1 ${finances?.cfoHired ? 'text-[#00c8ff]' : 'text-white'}`}>
                ...
                <button 
                  ...
                  className={`px-4 sm:px-8 py-3 sm:py-4 rounded-xl font-bold uppercase tracking-widest transition-all whitespace-normal sm:whitespace-nowrap text-xs sm:text-sm text-center
                    ${finances?.cfoHired 
                      ...
                      : 'bg-gradient-to-r from-[#00c8ff] to-[#0090b8] text-white hover:scale-105 shadow-[0_0_15px_rgba(0,200,255,0.4)]'}`}
                >
            ...
            {/* MANUEL ETKİNLİKLER */}
            {!finances?.cfoHired && (
              <div className="bg-[#141b2d]/80 rounded-2xl border border-white/5 p-6 sm:p-8">
                <div className="flex flex-col sm:flex-row gap-6 items-start sm:items-center justify-between mb-8 pb-8 border-b border-white/5">
                  ...
                  <div className="bg-black/50 border border-[#f5c842]/30 px-6 py-4 rounded-xl text-center">
                    <div className="text-[10px] text-[#f5c842] uppercase font-bold tracking-widest mb-1">Haftalık Hak</div>
                  ...
                  {[
                    ...
                    return (
                      <div key={ev.id} className="bg-black/30 border border-white/5 rounded-2xl p-5 hover:border-white/10 transition-colors flex flex-col">
                        ...
                        <div className="text-[10px] text-[#8892b0] uppercase font-bold mb-4 bg-white/5 inline-block px-2 py-1 rounded w-max">{ev.risk}</div>
                        ...
                        <div className="space-y-2 mb-6 flex-1">
                          ...
                          <div className="flex justify-between text-sm">
                            ...
                            <span className="text-[#00e676] font-bold">~ {formatMoney(ev.expected)}</span>
                          </div>
                        </div>
