...
export function MatchContainer() {
  ...
  if (!fixture || (fixture.played && !isFinished)) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[70vh]">
        ...
        <p className="text-muted-foreground mb-8">Takımınız bu haftayı bay geçiyor veya fikstür tamamlandı.</p>
        <button 
          onClick={() => router.push("/")}
          className="bg-gradient-to-r from-primary to-primary-dark text-white px-8 py-3 rounded-xl font-bold tracking-widest uppercase shadow-[0_0_20px_rgba(0,200,255,0.3)] hover:scale-105 transition-transform"
        >
          Ana Ekrana Dön
        </button>
      </div>
  ...
  if (isFinished) {
    return (
      ...
      >
        <div className="bg-card/90 backdrop-blur-xl rounded-3xl border border-white/10 shadow-[0_20px_60px_rgba(0,0,0,0.6)] overflow-hidden">
          <div className="p-8 text-center bg-black/40 border-b border-white/5 relative overflow-hidden">
            ...
            <h2 className="text-[10px] text-primary font-bold tracking-[4px] uppercase mb-2 relative z-10">Maç Sonucu</h2>
            ...
            <div className="flex items-center justify-center gap-8 md:gap-16 relative z-10 mt-6">
              ...
              <div className="flex flex-col items-center">
                ...
                <div className="mt-4 bg-white/10 px-4 py-1 rounded-full text-[10px] font-bold tracking-widest uppercase text-muted-foreground border border-white/5">
                  Tamamlandı
                </div>
              </div>
            ...
            {isAiPostMatchLoading ? (
              <div className="mt-8 text-center bg-black/40 p-4 rounded-xl border border-primary/20">
                <div className="animate-pulse text-primary font-rajdhani font-bold text-lg flex items-center justify-center gap-3">
              ...
              <div className="mt-8 bg-black/40 p-6 rounded-xl border border-primary/20 text-left relative overflow-hidden">
                <div className="absolute top-0 right-0 p-4 opacity-10 text-6xl">📰</div>
                <h3 className="font-rajdhani font-black text-2xl sm:text-3xl text-primary mb-3 leading-tight">{aiPostMatchReport.headline}</h3>
                <p className="text-foreground text-sm leading-relaxed" dangerouslySetInnerHTML={{ __html: aiPostMatchReport.report }}></p>
              </div>
            ) : null}
          </div>
          ...
          <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-8">
            {/* Stats */}
            <div>
              <h3 className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider mb-6 text-center border-b border-white/5 pb-2">Maç İstatistikleri</h3>
                ...
                {[
                  ...
                  <div key={i}>
                    ...
                    <div className="flex h-1.5 w-full bg-black rounded-full overflow-hidden">
                      <div className="h-full bg-primary" style={{ width: `${(stat.h / (stat.h + stat.a || 1)) * 100}%` }}></div>
                      <div className="h-full bg-destructive" style={{ width: `${(stat.a / (stat.h + stat.a || 1)) * 100}%` }}></div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {/* Highlights */}
            <div>
              <h3 className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider mb-4 text-center border-b border-white/5 pb-2">Önemli Anlar</h3>
              <div className="space-y-3 max-h-[250px] overflow-y-auto custom-scrollbar pr-2">
                {events.filter(e => e.type === 'goal' || e.type === 'red_card').map((e, idx) => (
                  <div key={idx} className="flex gap-3 items-center bg-black/20 p-3 rounded-lg border border-white/5">
                    <div className={`font-orbitron font-bold text-sm ${e.type === 'goal' ? 'text-green' : 'text-destructive'}`}>
                      {e.minute}'
                    </div>
                ...
                {events.filter(e => e.type === 'goal' || e.type === 'red_card').length === 0 && (
                  <div className="text-center py-8 text-muted text-sm">Gole veya kırmızı karta rastlanmadı.</div>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="p-6 bg-black/40 border-t border-white/5 flex flex-col sm:flex-row justify-center gap-4">
            ...
            <button 
              className="px-8 py-3 rounded-xl bg-gradient-to-r from-primary to-primary-dark text-white font-bold tracking-widest uppercase shadow-[0_0_20px_rgba(0,200,255,0.3)] hover:scale-105 transition-transform"
  ...
  if (showInterview) {
    ...
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="absolute inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
        <div className="bg-card border border-primary/30 w-full max-w-2xl rounded-3xl shadow-[0_0_50px_rgba(0,200,255,0.15)] overflow-hidden">
          <div className="bg-gradient-to-r from-primary/20 to-transparent p-6 border-b border-white/5 text-center">
            <h2 className="text-2xl font-orbitron font-bold text-white mb-2">🎤 Maç Sonu Basın Toplantısı</h2>
            <p className="text-muted-foreground text-sm">Medya mensupları maçın gidişatı hakkında sorular soruyor...</p>
          </div>
                  ...
                  {postMatchAiData?.options ? postMatchAiData.options.map((opt, i) => (
                    <button 
                      ...
                      className={`px-6 py-4 bg-white/5 hover:bg-white/10 text-white font-bold rounded-xl transition-colors border text-left flex items-center gap-3 ${
                        opt.type === 'positive' ? 'border-green/30' : opt.type === 'negative' ? 'border-destructive/30' : 'border-white/10 text-muted-foreground'
                      }`}
                    >
                    ...
                    <>
                      <button 
                        ...
                        className="px-6 py-4 bg-white/5 hover:bg-white/10 text-white font-bold rounded-xl transition-colors border border-green/30 text-left flex items-center gap-3"
                      >
                        <span className="text-xl">✅</span> Takımı öv ve destekle (Moral +)
                      </button>
                      <button 
                        onClick={() => answerInterview(false)}
                        className="px-6 py-4 bg-white/5 hover:bg-white/10 text-white font-bold rounded-xl transition-colors border border-destructive/30 text-left flex items-center gap-3"
                      >
                        <span className="text-xl">🔥</span> Takımı sert eleştir veya hakeme yüklen (Riskli)
                      </button>
                      <button 
                        onClick={() => answerInterview(null)}
                        className="px-6 py-4 bg-white/5 hover:bg-white/10 text-muted-foreground font-bold rounded-xl transition-colors border border-white/10 text-left flex items-center gap-3"
                      >
  ...
  return (
    <div className="pb-10 h-full flex flex-col">
      {/* Scoreboard Header */}
      <div className="bg-card/90 backdrop-blur-xl rounded-2xl border border-white/5 shadow-lg p-4 sm:p-6 mb-4 sm:mb-6">
        <div className="flex flex-wrap items-center justify-between gap-3 sm:gap-6">
          <div className="flex items-center gap-3 flex-1 justify-end min-w-0">
            <div className="text-right min-w-0">
              ...
              <div className="text-[9px] sm:text-[10px] text-muted-foreground uppercase tracking-widest">Ev Sahibi</div>
            </div>
          ...
          <div className="flex flex-col items-center mx-1 sm:mx-4">
            ...
            <div className="text-primary font-orbitron font-bold text-base sm:text-xl mt-1 sm:mt-2 animate-pulse">
              {minute}'
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 flex-1 justify-start min-w-0">
            ...
            <div className="text-left min-w-0">
              ...
              <div className="text-[9px] sm:text-[10px] text-muted-foreground uppercase tracking-widest">Deplasman</div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-6">
          <div className="h-1.5 w-full bg-black/50 rounded-full overflow-hidden border border-white/5">
            <div className="h-full bg-gradient-to-r from-primary to-[#00e676] transition-all duration-1000 ease-linear shadow-[0_0_10px_var(--color-primary)]" style={{ width: `${(minute / 90) * 100}%` }}></div>
          </div>
        </div>
        ...
        <div className="mt-6 flex justify-center gap-4 w-full">
          {!engine && !showPreMatchEvent && (
            <button 
              ...
              className={`px-8 py-3 rounded-xl font-bold tracking-widest uppercase flex items-center gap-2 transition-transform ${isAiLoading ? 'bg-gray-600 text-gray-400 cursor-not-allowed' : 'bg-gradient-to-r from-green to-[#00b25c] text-white shadow-[0_0_20px_rgba(0,230,118,0.3)] hover:scale-105'}`}
              onClick={startMatch}
            >
          ...
          {!engine && showPreMatchEvent && (
            <div className="bg-card p-6 rounded-2xl border border-yellow-500/50 shadow-[0_0_30px_rgba(234,179,8,0.2)] text-center w-full max-w-2xl mx-auto animate-pulse-slow">
          ...
          {engine && (
            <>
              <button 
                ...
                className="bg-gradient-to-r from-primary/20 to-primary-dark/20 text-primary hover:bg-primary/30 border border-primary/50 px-6 py-3 rounded-xl font-bold tracking-widest uppercase transition-colors shadow-[0_0_15px_rgba(0,200,255,0.15)]"
              >
              ...
              <button 
                ...
                className={`px-6 py-3 rounded-xl font-bold tracking-widest uppercase border transition-colors ${
                  isMatchPaused 
                    ? 'bg-green/20 text-green border-green/50 hover:bg-green/30' 
              ...
              <button 
                onClick={fastForward}
                className="bg-destructive/20 hover:bg-destructive/40 text-destructive border border-destructive/30 px-6 py-3 rounded-xl font-bold tracking-widest uppercase transition-colors"
              >
                ⏩ Hızlı Bitir
              </button>
            </>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 flex-1 min-h-[400px] sm:min-h-[500px]">
        ...
        <div className="lg:col-span-2 flex flex-col gap-6">
          {/* Pitch 2D */}
          <div className="bg-card/80 backdrop-blur-md rounded-2xl border border-white/5 shadow-lg p-4 flex justify-center items-center relative overflow-hidden h-[300px]">
            ...
            {!engine && minute === 0 && (
              <div className="absolute inset-0 z-20 flex items-center justify-center bg-black/40 backdrop-blur-sm">
                <div className="text-muted-foreground font-rajdhani font-bold text-xl tracking-widest uppercase">Maç Bekleniyor...</div>
              </div>
            )}
          </div>

          {/* Live Commentary Feed */}
          <div className="bg-card/80 backdrop-blur-md rounded-2xl border border-white/5 shadow-lg flex-1 flex flex-col min-h-[300px]">
            <div className="px-6 py-4 border-b border-white/5 bg-black/20">
              <span className="font-rajdhani font-bold text-lg tracking-wider text-foreground uppercase flex items-center gap-2">
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-3" ref={scrollRef}>
              <AnimatePresence>
                {[...events].reverse().map((e, idx) => (
                  <motion.div 
                    ...
                    className="flex gap-4 p-3 bg-black/20 rounded-xl border border-white/5 hover:border-primary/30 transition-colors"
                  >
                    <div className="font-orbitron font-bold text-primary w-8 shrink-0">{e.minute}'</div>
                    <div className="text-foreground text-sm leading-relaxed" dangerouslySetInnerHTML={{ __html: e.text }}></div>
                  </motion.div>
                ))}
              </AnimatePresence>
              {events.length === 0 && !engine && (
                <div className="h-full flex items-center justify-center text-center p-4">
                  {isAiLoading ? (
                    <div className="text-primary text-sm animate-pulse flex flex-col items-center gap-2">
                    ...
                    <div className="text-foreground text-sm italic bg-primary/10 p-4 rounded-xl border border-primary/20">
                      🎙️ "{aiData.preview}"
                    </div>
                  ) : (
                    <div className="text-muted text-sm">Maç başladığında önemli anlar burada akacak.</div>
                  )}
                </div>
              )}
            </div>
        ...
        <div className="bg-card/80 backdrop-blur-md rounded-2xl border border-white/5 shadow-lg flex flex-col">
          <div className="px-6 py-4 border-b border-white/5 bg-black/20">
            <span className="font-rajdhani font-bold text-lg tracking-wider text-foreground uppercase">Anlık İstatistikler</span>
          </div>
      ...
      {(isHalfTime || injuryPause || isUserPaused) && (
        <div className="fixed inset-0 z-50 bg-black/90 backdrop-blur-md flex items-center justify-center p-4 sm:p-6">
          <div className="bg-secondary w-full max-w-[1200px] h-full max-h-[92vh] flex flex-col rounded-3xl border border-primary/30 shadow-[0_0_60px_rgba(0,200,255,0.15)] relative overflow-hidden">
            
            {/* Dekoratif Efekt */}
            <div className={`absolute top-0 inset-x-0 h-32 bg-gradient-to-b ${injuryPause ? 'from-red-500/20' : 'from-primary/10'} to-transparent pointer-events-none`}></div>
            ...
            <div className="p-5 sm:p-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-white/10 bg-black/20 flex-shrink-0 relative z-10">
              <div>
                <h2 className={`text-2xl sm:text-4xl font-rajdhani font-black text-transparent bg-clip-text bg-gradient-to-r ${injuryPause ? 'from-red-500 to-red-300' : 'from-white to-primary'} uppercase tracking-widest leading-none`}>
            ...
            <div className="flex flex-col lg:flex-row flex-1 overflow-y-auto lg:overflow-hidden p-5 sm:p-8 gap-6 sm:gap-8 relative z-10 custom-scrollbar">
              ...
              <div className="flex-1 flex flex-col bg-black/30 rounded-2xl border border-white/5 overflow-hidden min-h-[400px] lg:min-h-0 flex-shrink-0 lg:flex-shrink">
                <div className="p-4 sm:p-5 bg-black/40 border-b border-white/5 flex items-center justify-between flex-shrink-0">
                  <h3 className="text-primary font-rajdhani font-bold text-sm sm:text-base uppercase tracking-wider">Sahadakiler</h3>
                      ...
                      return (
                        ...
                        >
                          <div className="flex flex-col justify-center items-center w-10 sm:w-14">
                            <div className="font-orbitron font-bold text-xs sm:text-sm text-muted-foreground">{p.position}</div>
                            {p.alternatePositions && p.alternatePositions.length > 0 && (
                              <div className="text-[7px] sm:text-[8px] text-muted uppercase tracking-tighter truncate w-full text-center mt-0.5">
                          ...
                          <div className="w-16 sm:w-20 flex flex-col gap-1 items-end">
                            <span className={`text-[9px] font-bold ${fitness < 60 ? 'text-red-400' : fitness < 80 ? 'text-yellow-400' : 'text-green'}`}>
                              %{fitness}
                            </span>
                            <div className="w-full h-1 bg-black/50 rounded-full overflow-hidden">
                              <div className={`h-full ${fitness < 60 ? 'bg-red-500' : fitness < 80 ? 'bg-yellow-500' : 'bg-green'}`} style={{ width: `${fitness}%` }}></div>
                            </div>
                          </div>

                          <div className="text-primary font-bold text-sm w-6 text-right">{p.overall}</div>
                        </div>
                      );
                    })}
                  </div>
              ...
              <div className="flex-1 flex flex-col gap-6 sm:gap-8 lg:overflow-hidden flex-shrink-0 lg:flex-shrink">
                ...
                <div className="flex-1 flex flex-col bg-black/30 rounded-2xl border border-white/5 overflow-hidden min-h-[400px] lg:min-h-0">
                  <div className="p-4 sm:p-5 bg-black/40 border-b border-white/5 flex items-center justify-between flex-shrink-0">
                    <h3 className="text-primary font-rajdhani font-bold text-sm sm:text-base uppercase tracking-wider">Yedek Kulübesi</h3>
                    <span className="text-[10px] sm:text-xs bg-green/20 text-green px-3 py-1 rounded-md font-bold uppercase">Oyuna Al</span>
                  </div>
                  <div className="flex-1 overflow-y-auto custom-scrollbar p-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2 gap-3">
                      {squad.filter(id => !currentLineup.includes(id)).map(pid => {
                        ...
                        return (
                          <div 
                            ...
                            className={`flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-all border ${isSelected ? 'bg-green/20 border-green/50 shadow-[0_0_15px_rgba(0,230,118,0.2)]' : 'bg-white/5 border-transparent hover:bg-white/10'}`}
                          >
                            <div className="flex flex-col justify-center items-center w-10 sm:w-14">
                              <div className="font-orbitron font-bold text-xs sm:text-sm text-muted-foreground">{p.position}</div>
                              {p.alternatePositions && p.alternatePositions.length > 0 && (
                            ...
                            <div className="w-16 sm:w-20 flex flex-col gap-1 items-end">
                              <span className={`text-[9px] font-bold ${fitness < 60 ? 'text-red-400' : fitness < 80 ? 'text-yellow-400' : 'text-green'}`}>%{fitness}</span>
                              <div className="w-full h-1 bg-black/50 rounded-full overflow-hidden">
                                <div className={`h-full ${fitness < 60 ? 'bg-red-500' : fitness < 80 ? 'bg-yellow-500' : 'bg-green'}`} style={{ width: `${fitness}%` }}></div>
                              </div>
                            </div>
                ...
                {/* Taktik Paneli */}
                <div className="bg-black/40 rounded-2xl border border-primary/20 p-5 sm:p-6 flex-shrink-0">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-5">
                    ...
                    <div className="flex gap-2 w-full sm:w-auto">
                      <button
                        ...
                        className="bg-gradient-to-r from-gold/20 to-[#d4a017]/20 border border-gold/50 text-gold hover:bg-gold/30 px-4 py-3 rounded-xl text-xs font-bold uppercase tracking-wider flex-1 sm:flex-none shadow-[0_0_15px_rgba(245,200,66,0.1)] transition-colors"
                      >
                        ⚡ Otomatik
                      </button>
                      <button 
                        ...
                        className="bg-primary text-black px-4 sm:px-6 py-3 rounded-xl text-xs sm:text-sm font-bold uppercase tracking-wider disabled:opacity-30 disabled:cursor-not-allowed hover:bg-white transition-all shadow-[0_0_15px_rgba(0,200,255,0.2)] flex-1 sm:flex-none"
                      >
                        Onayla
                      </button>
                    </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <select 
                      ...
                      className="bg-secondary text-white border border-white/10 rounded-xl p-3 text-sm font-rajdhani outline-none focus:border-primary transition-colors"
                    >
