...
class MatchEngine {
  ...
  _calcTeamRating(squad, tactics) {
    ...
    let tacticalBonus = 0;
    if (tactics.style === 'attacking') tacticalBonus += 1;
    if (tactics.style === 'defensive') tacticalBonus -= 1; // Defansif taktik atağı düşürür, savunmayı artırır ama rating'i dengede tutar. İleride şut ihtimalini etkileyecek.
    if (tactics.press === 'high') tacticalBonus += 1.5; // Yüksek pres gücü artırır ama ileride yorgunluk eklenebilir.
  ...
  _simulateMinute() {
    ...
    if (this.homeTactics.style === 'attacking') homeStrength += 4;
    if (this.homeTactics.style === 'defensive') homeStrength -= 3; // Daha az atak şansı
    if (this.awayTactics.style === 'attacking') homeStrength -= 4; // Rakip ataksa bizim defans zorlanır
    if (this.awayTactics.style === 'defensive') homeStrength += 3; // Rakip defansifse bizim baskımız artar
    ...
    // Update possession gradually (Fixed Math.round trap)
    if (!this._internalPossession) this._internalPossession = 50.0;
    this._internalPossession = this._internalPossession * 0.90 + (isHomeAction ? 65 : 35) * 0.10;
