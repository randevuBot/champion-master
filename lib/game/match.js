...
class MatchEngine {
  constructor(homeTeam, awayTeam, homeSquad, awaySquad, homeTactics = null, awayTactics = null, aiLines = null, homeStaff = null, awayStaff = null) {
    this.home = homeTeam;
    this.away = awayTeam;
    ...
    this.homeRating = this._calcTeamRating(homeSquad, this.homeTactics, this.homeStaff);
    this.awayRating = this._calcTeamRating(awaySquad, this.awayTactics, this.awayStaff);
  }

  _calcTeamRating(squad, tactics, staff = null) {
    ...
    // Maç Öncesi Prim (Moral) Bonusu
    if (tactics.preMatchBoost) {
      tacticalBonus += 5; // Motivasyon ciddi bir avantaj sağlar
    }

    // Takım Uyumu (Chemistry) Etkisi
    if (tactics.chemistry !== undefined) {
      if (tactics.chemistry < 50) tacticalBonus -= 4;
      else if (tactics.chemistry < 70) tacticalBonus -= 1.5;
      else if (tactics.chemistry >= 90) tacticalBonus += 3;
      else if (tactics.chemistry >= 80) tacticalBonus += 1;
    }
  ...
  _tryFoul(isHome) {
    ...
    if (r < redChance) {
      if (isHome) {
        ...
        this.homeSquad = this.homeSquad.filter(p => p.id !== fouler.id);
      } else {
        this.stats.redCards.away++;
        this.awaySquad = this.awaySquad.filter(p => p.id !== fouler.id);
      }
